...
export async function POST(request: NextRequest) {
  try {
    ...
    let response = await fetch('https://cool-starfish-suitable.ngrok-free.app/text2speech/', {
      method: 'POST',
      body: formData,
    });
    ...
    if (response.status === 422 || response.status === 415) {
      ...
      response = await fetch('https://cool-starfish-suitable.ngrok-free.app/text2speech/', {
        method: 'POST',
        headers: {
